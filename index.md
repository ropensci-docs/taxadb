...
gives each provider’s preferred citation):
...
we find this is because the name resolves not to zero matches, but is a
known synonym to more than one accepted name (as indicated by the
...
[`get_names()`](https://docs.ropensci.org/taxadb/reference/get_names.md)
on the *accepted* IDs: (These also correspond to the genus and
specificEpithet column, as the classification is always given only based
on acceptedNameUsageID).
...
[`get_ids()`](https://docs.ropensci.org/taxadb/reference/get_ids.md)
returns one identifier per input name, so it has to return `NA` when a
name resolves more than one way, with a warning telling you which name
was ambiguous:
...
better query – which is the honest answer, and the reason
[`get_ids()`](https://docs.ropensci.org/taxadb/reference/get_ids.md)
...
[`taxa_tbl()`](https://docs.ropensci.org/taxadb/reference/taxa_tbl.md)
function (or directly accessing the database connection using
[`td_connect()`](https://docs.ropensci.org/taxadb/reference/td_connect.md))
is more low-level and requires greater care. For instance, we cannot
just add a
...
Instead, we would first need to use a
[`collect()`](https://dplyr.tidyverse.org/reference/compute.html) to
pull the summary table into memory. Users familiar with remote databases
in `dplyr` will find using
[`taxa_tbl()`](https://docs.ropensci.org/taxadb/reference/taxa_tbl.md)
...
- [`taxadb_provider_info()`](https://docs.ropensci.org/taxadb/reference/taxadb_provider_info.md)
  gives each provider’s authority, licence and preferred citation. Note
  that `fb` and `slb` are CC BY-NC.
...
checks the rules the tables are supposed to satisfy: `scientificName`
non-empty at every rank, `acceptedNameUsageID` populated on accepted
names as well as synonyms, every `acceptedNameUsageID` resolving to an
accepted name, identifiers carrying the provider prefix and never naming
two different names. It is worth running on your own builds, and it is
what gates the published ones.

Builds run entirely in `duckdb` and out of core, so they are bounded by
disk rather than memory. See
